...
class InterviewMCP:
    ...
    async def call_tool(self, params: Dict[str, Any]) -> Any:
        ...
        if name == "fetch_interview_context":
            ...
            # ✅ Format transcript with *Agent* and *Candidate*
            formatted_transcript = ""
            for line in transcript.split("\n"):
                if line.strip().lower().startswith("agent:"):
                    formatted_transcript += f"*Agent*: {line.split('Agent:',1)[1].strip()}\n\n"
                elif line.strip().lower().startswith("candidate:"):
                    formatted_transcript += f"*Candidate*: {line.split('Candidate:',1)[1].strip()}\n\n"
                else:
                    formatted_transcript += line.strip() + "\n\n"
...
=== CANDIDATE RESUME ===
{context.get('resume_text', 'Resume not available')}
